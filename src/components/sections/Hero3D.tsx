...
const slides = [
  ...
  },
  {
    ...
    brand: "Premium",
    model: "Frames",
    desc: "We craft premium custom frames that preserve and showcase your cherished memories, artwork, and photographs with elegance.",
    img: "/heroimg/frame.png",
  },
  {
    id: "07",
    brand: "Custom",
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          maskImage:
            "linear-gradient(to right, rgba(0,0,0,1) 0%, rgba(0,0,0,0.8) 30%, rgba(0,0,0,0) 80%)",
          WebkitMaskImage:
            "linear-gradient(to right, rgba(0,0,0,1) 0%, rgba(0,0,0,0.8) 30%, rgba(0,0,0,0) 80%)",
        }}
      >
      ...
      <div className="absolute right-6 md:right-12 top-1/2 -translate-y-1/2 z-20 flex flex-col items-end justify-center py-10 pointer-events-auto">
        {slides.map((s, i) => (
          <React.Fragment key={s.id}>
            ...
            >
              <span
                className={`font-space transition-all duration-300 ${i === currentIndex ? "text-black text-sm font-bold" : "text-gray-400 text-xs font-medium group-hover:text-gray-600"}`}
              >
                {s.id}
              </span>
              <div
      ...
      <div className="absolute bottom-5 md:bottom-12 left-4 md:left-12 flex gap-2 md:gap-3 z-30 pointer-events-auto">
        {slides.map((s, i) => (
          ...
          >
            <div
              className={`w-10 h-10 md:w-16 md:h-16 rounded-xl overflow-hidden bg-white/50 backdrop-blur-md border transition-all duration-500 flex items-center justify-center ${i === currentIndex ? "border-gray-800 shadow-xl scale-110" : "border-transparent opacity-50 hover:opacity-100"}`}
            >
              <img
                src={s.img}
                alt={s.brand}
