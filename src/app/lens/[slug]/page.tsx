  ...
  if (!lens) {
    return (
      <main className="min-h-screen bg-background pt-40 pb-24 flex items-center justify-center">
        <h1 className="font-syncopate text-2xl">lens Not Found</h1>
      </main>
    );
  }

  const activeColor = lens.colors[activeColorIdx];
  const nextImage = () =>
    setCurrentImage((prev) => (prev + 1) % activeColor.images.length);
  const prevImage = () =>
      ...
      <main className="min-h-screen bg-backgroundd pt-40 pb-24">
        <div className="max-w-350 mx-auto px-6 md:px-12">
          ...
          >
            ...
            >
              <ArrowLeft className="w-3 h-3" /> Back to lens
            </Link>
            <span>/</span>
            <span className="capitalize">{lens.category}</span>
            <span>/</span>
            <span className="text-[#111] font-medium">{lens.name}</span>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            >
              <div className="bg-[#F5F5F5] border border-transparent rounded-sm p-12 flex items-center justify-center min-h-125 relative overflow-hidden shadow-sm group">
                ...
                >
                  <AnimatePresence mode="wait">
                    <motion.img
                      ...
                      alt={`${lens.name} - ${activeColor.name}`}
                ...
                <button
                  onClick={prevImage}
                  className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white border border-gray-200 rounded-sm flex items-center justify-center shadow-md hover:bg-[#111] hover:text-white transition-colors z-20 group"
                >
                  <ChevronLeft className="w-5 h-5 group-hover:-translate-x-0.5 transition-transform" />
                </button>
                <button
                  onClick={nextImage}
                  className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white border border-gray-200 rounded-sm flex items-center justify-center shadow-md hover:bg-[#111] hover:text-white transition-colors z-20 group"
                >
            ...
            >
              ...
              {/* Color Selection */}
              {lens.colors && lens.colors.length > 0 && (
                <div className="mb-8">
              ...
              <ul className="space-y-3">
                {lens.keyFeatures?.map((feature, i) => (
                  ...
                  <>
                    ...
                    <ul className="space-y-3">
                      {lens.keyFeatures.map((feature, i) => (
                        <li
                          key={i}
                          className="flex items-start gap-3 font-space text-sm text-gray-600"
                        >
                          <span className="text-[#111] mt-0.5 opacity-40">
                            ■
                          </span>
                          {feature}
                        </li>
          ...
          {/* {lens.richFeatures && lens.richFeatures.length > 0 && (
          <div className="mt-32 border-t border-gray-300 pt-24">
            ...
            <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-20">
              {lens.richFeatures.map((feat, idx) => (
                <div 
                  key={idx}
                ...
                {lens.specs.map((specGroup, idx) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`w-5 h-5 text-gray-500 transition-transform duration-300 ${openSpecCategory === specGroup.category ? "rotate-180" : ""}`}
                      />
                    </button>
