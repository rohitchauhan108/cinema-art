  ...
  const filteredlens = lens.filter((lens) => {
    ...
    const matchesCategory =
      activeCategory === "All Lenses"
        ...
        : lensCategories.includes(activeCategory);
    const matchesBrand =
      selectedBrands.length === 0 || selectedBrands.includes(lens.brand);
    const matchesSearch =
        ...
        <div className="max-w-350 mx-auto px-6 md:px-12">
          ...
          >
            ...
            <p className="font-space mt-4 text-gray-600 max-w-xl">
              Capture every detail with premium lenses featuring advanced
              optics, precision engineering, and superior performance.
            </p>
          </motion.div>
                  ...
                  filteredlens.map((lens, index) => (
                    ...
                    >
                      ...
                      <div className="relative h-60 w-full bg-[#F5F5F5] flex items-center justify-center p-8 overflow-hidden">
                        ...
                        <img
                          src={lens.colors[0].images[0]}
                          alt={lens.name}
                          className={`max-h-full max-w-full object-contain mix-blend-multiply group-hover:scale-110 transition-transform duration-700 ease-out drop-shadow-xl ${getPrimaryCategory(lens) === "Action Cameras" || getPrimaryCategory(lens) === "360 Cameras" ? "p-4" : ""}`}
                        />
                      ...
                      <div className="pt-5 pb-5 px-5 flex flex-col flex-1 bg-white z-10 relative border-t border-gray-100">
                        ...
                        <div className="mt-auto flex items-end justify-between">
                          <Link
                            href={`/lens/${getPrimaryCategory(lens).toLowerCase().replace(/ & | /g, "-")}/${lens.slug}`}
